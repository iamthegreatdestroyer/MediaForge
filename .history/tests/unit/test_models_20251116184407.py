...
async def test_relationships(db: Database):
    async with db.session() as session:
        ...
        meta = MediaMetadata(media_item=item, duration=10.5, width=1920, height=1080)
        tag = Tag(name="Sample")
    ...
    async with db.session() as session:
        ...
        assert loaded.metadata.duration == 10.5
        assert loaded.tags[0].name == "Sample"
        assert loaded.collections[0].name == "Set A"


@pytest.mark.asyncio
async def test_indexes_exist(db: Database):
    inspector = inspect(db.engine)
    indexes = inspector.get_indexes("media_items")
