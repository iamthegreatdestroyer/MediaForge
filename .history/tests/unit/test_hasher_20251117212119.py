...
async def test_hash_file(sample_video_file):
    """Test file hashing"""
    hasher = FileHasher()
    file_hash = await hasher.hash_file(sample_video_file)
...
async def test_hash_consistency(sample_video_file):
    ...
    hash1 = await hasher.hash_file(sample_video_file)
    hash2 = await hasher.hash_file(sample_video_file)
...
async def test_different_files_different_hashes(
    ...
    hash1 = await hasher.hash_file(sample_video_file)
    hash2 = await hasher.hash_file(sample_audio_file)
...
async def test_hash_nonexistent_file(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        await hasher.hash_file(nonexistent)
...
async def test_hash_empty_file(tmp_path):
    ...
    file_hash = await hasher.hash_file(empty_file)
    assert file_hash is not None
    assert len(file_hash) > 0
