...
class TagBase(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=128, description="Tag name"
    )
    description: Optional[str] = Field(
        None, max_length=512, description="Tag description"
    )
...
class TagReadWithMediaCount(TagRead):
    ...
    media_count: int = Field(
        default=0, description="Number of media items with this tag"
    )
