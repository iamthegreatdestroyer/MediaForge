...
class MediaItem(UUIDMixin, TimestampMixin, Base):
    ...
    file_hash: Mapped[str] = mapped_column(String(128), unique=True, index=True, nullable=False)
    mime_type: Mapped[str] = mapped_column(String(128), index=True, nullable=False)
    media_type: Mapped[MediaType] = mapped_column(SAEnum(MediaType), index=True, nullable=False)
    ...
    metadata: Mapped["MediaMetadata" | None] = relationship(
        "MediaMetadata", back_populates="media_item", cascade="all, delete-orphan", uselist=False
    )
    ...
    __table_args__ = (
        Index("ix_media_items_media_type_created_at", "media_type", "created_at"),
    )
