...
class AudioMetadataExtractor:
    ...
    Notes:
        Mutagen provides different tag schemas per format (ID3, MP4, FLAC, etc.).
        We normalize common fields and attempt multiple keys for each logical field.
    """

    async def extract(self, file_path: Path) -> Dict[str, Any]:
        """Extract audio tags and technical metadata.

        Returns a dictionary with normalized tag names and technical info.
        """
        try:
            loop = asyncio.get_event_loop()
            # Use keyword easy for better cross-format simple tags when available
            audio = await loop.run_in_executor(
                None, lambda: MutagenFile(str(file_path), easy=True)
            )
            if audio is None:
                raise MetadataExtractionError("Could not read audio file")

            # Build tag dictionary with lowercase keys for uniform access
            tag_map: Dict[str, Any] = {}
            try:
                for k in list(audio.keys()):  # type: ignore[attr-defined]
                    lk = str(k).lower()
                    tag_map[lk] = audio[k]
            except Exception:
                pass

            def get_first(*possible: str) -> Optional[str]:
                for name in possible:
                    v = tag_map.get(name.lower())
                    if v is None:
                        # Fall back to direct mapping when keys() not available on mock
                        try:
                            if name in audio:  # type: ignore[operator]
                                v = audio[name]  # type: ignore[index]
                        except Exception:
                            v = None
                    if v is None:
                        continue
                    # Mutagen may store list-like values
            ...
            # Logical tags with fallbacks (ID3 frame names included where applicable)
            # Include broader fallbacks seen in some tagging tools
            artist = get_first('artist', 'albumartist', 'performer', 'author', 'tpe1', 'tpe2')
            album = get_first('album', 'talb')
            title = get_first('title', 'tit2')
            date_raw = get_first('date', 'year', 'tdrc')
            genre = get_first('genre', 'tcon')
            track_raw = get_first('tracknumber', 'track', 'trck')
            disc_raw = get_first('discnumber', 'disc', 'tpos')
            composer = get_first('composer', 'tcom')
            comment = get_first('comment', 'tcom')  # tcom reused if generic comment missing
            lyrics = get_first('lyrics', 'unsyncedlyrics', 'uslt')
            publisher = get_first('publisher', 'tpub')
            encoder = get_first('encoder', 'tenc')

            metadata: Dict[str, Any] = {
                'artist': artist,
                'album': album,
                'title': title,
                'year': self._parse_year(date_raw),
                'genre': genre,
                'track_number': parse_number(track_raw),
                'disc_number': parse_number(disc_raw),
                'composer': composer,
                'comment': comment,
                'lyrics': lyrics,
                'publisher': publisher,
            ...
            raise MetadataExtractionError(f"Error reading audio tags: {e}")

    # Retain compatibility for year parsing and track parsing helpers
    def _get_tag(self, audio: Any, key: str) -> Optional[str]:  # legacy helper (used by tests maybe)
        try:
            if key in audio:  # type: ignore
                value = audio[key]
